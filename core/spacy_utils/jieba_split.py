...
def load_chinese_stopwords():
    """从配置文件加载中文停用词表"""
    try:
        stopwords_path = load_key('chinese_stopwords_file', 'files/chinese_stopwords.txt')
...
# --------------------
# 1. split_by_mark_jieba
# --------------------

def split_by_mark_jieba():
    """
    按标点符号分割中文文本（对应 Spacy 版本的 split_by_mark）
    处理连字符连接（...、-）
    """
...
def count_chinese_words(text: str) -> int:
    ...
    # 排除标点和停用词
    meaningful_words = [w for w in words if w.strip() and w not in string.punctuation and w not in stopwords]
    return len(meaningful_words)


def split_by_comma_jieba_main():
    """
    按逗号分割中文句子（对应 Spacy 版本的 split_by_comma_main）
    检查左右是否构成完整句子（>=3 词）
    """
...
def split_sentences_jieba_main():
    """
...
def split_long_by_root_jieba_main():
    """
    ...
    rprint(f"[green]💾 Long sentences split by root (jieba) saved to → `{_3_1_SPLIT_BY_NLP}`[/green]")
