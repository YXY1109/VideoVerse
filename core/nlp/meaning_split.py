...
def parallel_split_sentences(sentences: list,language:str,nlp=None, max_length: int=42, use_jieba: bool = False) -> list:
...
def split_sentence(sentence, num_parts, word_limit=20, index=-1, retry_attempt=0):
    ...
    response_data = ask_llm(split_prompt + " " * retry_attempt, resp_type='json', valid_def=valid_split,
...
if __name__ == '__main__':
    result_zh=['小姐，我们来对我们本章节的内容做一个总结', '首先，我们本章节的主题是异步IO和携程本章节，我们引出了携程，', '但是到目前为止，我们还没有把携程应用到我们的具体代码当中，这里边', '我们的携程一般是需要配合我们的事件循环来使用的', '我们携程单独使用的话，实际上它的作用并不是很明显，', '而且它使用起来很不方便', '关于如何在事件循环中使用携程', '我们下一章的SyncIO将会给大家介绍到', '我们现在来回顾一下，我们本章节主要的目的是要由浅入深地给大家引出携程它的一个概念', '首先，我们来回顾一下本章节的一个主要内容，在最开始的时候，我们介绍了一些概念', '这里边包括并发并行同步亦步阻塞和非阻塞这些概念', '紧接着我们讲解了C十K问题和IO多路复用', '这里边的Io多路复用，包括我们的和易破本节课', '给大家详细地讲解了UNIX网络编程中的五种Io模型', '以及我们引出了我们的Io多路复用', 'Io多路复用是我们目前为止使用到的最多的一种技术', '我们高并发中使用的Io多路复用的技术非常之多', '所以说，我们本章节主要就是围绕着我们的Io多路复用来讲解的', '艾欧多洛夫用之后，我们就讲解了如何去使用的', '或者说一破加上我们的回调和事件循环的方式去请求我们的U二院', '在讲解这种回调和事件循环之前', '首先给大家讲解了如何去使用非阻塞的方法', '去实现我们的U二元请求', '然后我们再引出了我们的回调加事件循环', '通过第三章节，我相信大家也体会到了这种编程模式', '它和传统的同步L的编程模式差异很大', '所以说，我希望大家能够彻底搞清楚，我们这种编程的模式', '因为我们异步IO的模型当中', '它实际上都是采用了这种模式，包括NodeJS', '包括Python中的几乎所有的异步IO的框架', '以及我们加瓦里边的内地', '它们实际上都是采用这种模式', '这里边这个模式，它的核心在于事件循环加上回调', '然后我们讲解了回调之痛，', '如果我们去采用上面的模式，将它应用到编程当中', '实际上我们的编码过程是非常痛苦的', '第一个就是站的撕裂的问题', '整个调用站如果出现异常就很麻烦', '第二个就是嵌套台', '这让我们代码维护起来非常的头疼', '如果大家写过JS的话，就会知道JS里面有个事件监听', '它里边采用的就是回调模式', '实际上代码维护起来是很痛苦的', '所以说我们后边就引出了携程', '携程它本身它并不比我们的事件循环和回调这种编码方式性能高，反而它可能会比这个低一些', '但是携程主要解决的问题就是回调之痛的问题', '它让我们可以将事件循环加上L多路复用方法和我们的传统的同步编程模式结合起来', '携程的主要目的是为了解决整个编码习惯的一个问题', '然后我们讲解了生成器里边的剩的克洛斯吃肉以及我们的生成器当中的亚的放', '讲解了这些知识，就可以知道实际上可以利用生成器的一些原理来完成将生成器变成我们的携程这么一个过程', '这样的话，致辞整个编码', '它就可以兼顾我们的事件循环', 'IO多路复用以及同步编码思维结合起来', '然后我们后期再讲解了而Sync和而位置这两个关键词，这两个关键词，它使得最开始，大家注意一下', '在而Sync和而位置这两个关键词出现之前，整个携程', '它是利用了我们生成器的一些特性，将生成器变成我们的携程', '但是有了Sync和Avid之后，整个Python就开始支持我们的原生系统，大家在看以前的一些书籍，或者说以前的一些资料的时候', '很多地方都会讲一个装饰器就是coding', '它可以将我们的生成器装饰成写成，', '但是这里边我强烈建议大家', '如果大家使用得出Python的新版本', '就不要再去使用以前的克罗汀中式器的模式', '尽量的来采用而适应可和而为的这两个关键词', '这样的话，我们整个代码的含义是非常明确的，', '如果还在使用装饰器，加上生成器来实现我们的携程的话', '整个代码的后期维护实际上是会让大家很头疼的', '派森既然给我们提供了而适应和就要使用它的原生携程', '当然，这里边并没有说明这原生形成它的性能，或者说它其他方面就比我们的生成器这种模式好，它们俩达到的效果都是一样的，但是从语义上来讲', '以及从代码的后期维护来讲，是建议大家使用而而而为的', '因为我们生成器它实际上还有自己的一个用途', '如果大家很想了解这个装饰器背后的原理的话，大家可以进源码看一下，它的内容并不多', '实际上是去设置了我们的它的一些等等一些参数的', '设置这些参数之后，再去检查我们的一个函数，实际上就可以检查出来，它是不是我们的系统，这个过程实际上并不复杂', '在拍摄三点五', '三点六以及我们本课程的三点七当中', '我都会给大家讲解用而适应和尔维特来实现我们的系统，在前面讲解了那么多', '只是为了加深大家对我们携程的一个理解，', '因为我如果一上来就讲解而适应和尔维特的话，我相信绝大部分人是听不懂的', '本章节，我们关于易波IO和携程就介绍到这里，下一章节', '我们将开始来具体通过SyncIO框架来进一步学习我们的携程，好，本章节就到这里，谢谢大家']
