...
def create_custom_voice(audio_path, text, custom_name=None):
    ...
    response = requests.post(API_URL_VOICE, json=payload,
                             headers={"Authorization": f'Bearer {load_key["sf_fish_tts"]["api_key"]}',
...
def siliconflow_fish_tts_for_videolingo(text, save_as, number, task_df):
    ...
    if MODE == "preset":
        ...
        return siliconflow_fish_tts(text=text, save_path=save_as, mode="dynamic", ref_audio=str(ref_audio_path),
                                    ref_text=ref_text)
    else:
...
if __name__ == '__main__':
    ...
    siliconflow_fish_tts("가을 나뭇잎이 부드럽게 떨어지는 생생한 색깔을 주목하지 않을 수 없었다", "preset_test.wav", mode="preset",
                         check_duration=True)
